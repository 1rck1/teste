...
def calcular():
    if request.method == "POST":
            ...
            desconto = float(request.form.get('desconto'))
            valorINSS = calculaINSS(sal) 
            valorIRRF = calculaIRRF(sal, dep, valorINSS)
            salarioLiq = sal - (valorINSS + valorIRRF + desconto)
            descontoTotal =(valorIRRF + valorINSS + desconto)
            descontoPorcem = (descontoTotal * 100 / sal)
            r = 'R$'
            pc = '%'
            parA = '('
            parF = ')'
    ...
    return render_template('index.html', sal=sal, dep=dep, desconto=desconto, valorINSS=valorINSS, valorIRRF=valorIRRF, salarioLiq=salarioLiq, descontoPorcem=descontoPorcem, descontoTotal=descontoTotal, r=r, pc=pc, parA=parA, parF=parF)
